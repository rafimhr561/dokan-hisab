...
def create_transaction(user_id: int, tx: dict):
    ...
    c.execute("""
        ...
        (user_id, transaction_date, customer_name, service_type_id, service_type_name,
         amount, payment, status, reference, notes)
        VALUES (?,?,?,?,?,?,?,?,?,?)
    """, (
        user_id,
        tx["transaction_date"],
...
def update_transaction(tid: int, tx: dict):
    ...
    c.execute("""
        UPDATE transactions SET
            transaction_date=?, customer_name=?, service_type_name=?,
            amount=?, payment=?, status=?, reference=?, notes=?
        WHERE id=?
    """, (
        tx["transaction_date"],
